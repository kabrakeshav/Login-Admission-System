    ...
    if(!empty($_POST['pass'])  && !empty($_POST['idd']) && !empty($_POST['ffname'])) {
        ...
        if($numrows!=0)
        {
            ...
            // {
                ...
                // if($user=='$xfname' && $id==$xid){
                    $flag = 1;
                    $q = "DELETE FROM student_info WHERE id=$xid AND fname='$xfname'";
                    $status = mysqli_query($con, $q);
                    if($status == 1){
                        echo "Removed successfully from DB... Go to 'image_student' folder to delete photo...";
                    }else{
...
<html>
    ...
    <div class="controls" style="padding: 180px; margin-left: 300px">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    Id : <input value="<?php if(!empty($x)) echo("$x");?>" type="integer" name="id" class="form-control" placeholder="Enter id of student">
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    Student Name : <input value="<?php if(!empty($x)) echo("$fname");?>" id="form_name" type="text" name="user" class="form-control" placeholder="Enter First-Name of Student">
                </div>
            </div>
        </div>

        <button type="submit" value="delete" name="delete" class="btn btn-danger">Remove Student</button>
    </div>
    </form>
